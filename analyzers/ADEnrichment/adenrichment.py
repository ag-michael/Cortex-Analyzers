...
class ADEnrichment(Analyzer):
    def __init__(self):
        ...
        self.adurl = self.get_param(
            'config.ldapsurl',
            None,
            'LDAPS URL parameter is missing')
        self.domain = self.get_param(
            'config.domain', None, 'Domain parameter is missing')
        self.computer_basedn = self.get_param(
            'config.computer_basedn',
            None,
            'computer_basedn parameter is missing. This should be a semicolon separated list of base distinguished names such as ",OU=Mycompany,OU=Workstations,DC=corp,DC=local"')
        self.person_basedn = self.get_param(
            'config.person_basedn',
            None,
            'person_basedn parameter is missing. This should be a semicolon separated list of base distinguished names such as ",OU=Branch,OU=UserAccounts,DC=corp,DC=local;,OU=HQ,OU=UserAccounts,DC=corp,DC=local"')
        self.service_account = self.get_param(
            'config.serviceaccount',
            None,
            'serviceaccount parameter is missing. This account is used to bind to LDAP')
        self.service_account_password = self.get_param(
            'config.serviceaccount_password',
            None,
            'serviceaccount_password parameter is missing')
